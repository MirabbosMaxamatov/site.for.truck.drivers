...
(() => {
  ...
  function normalizeTextForParsing(str) {
    return String(str || "")
      .normalize("NFKD")
      .replace(/[\u0300-\u036f]/g, "")
      .replace(/[\uD835][\uDC00-\uDFFF]/g, (char) => {
  ...
  function parseUniversalTelegram(rawText) {
    if (!rawText) return null;

    // 1. Unicode & Styled Font Normalization
    ...
    const labeledMatch = cleanText.match(
      /(?:Trip\s*ID|Trip|Load|Order|PRO|Ref)\s*[:#-]?\s*([A-Z0-9-]{3,25})/i,
    );

    if (labeledMatch && labeledMatch[1]) {
      tripId = labeledMatch[1].trim();
    } else {
      const lines = cleanText.split("\n");
      for (let line of lines) {
        let sanitized = line.replace(/^[📍🗺️#\d\s:]+/, "").trim();
        const candidate = sanitized.split(/\s+/)[0];
        if (
          /^[A-Z0-9-]{3,20}$/i.test(candidate) &&
          !/^(LOADED|EMPTY|CANCELLED|TONU|EST|CDT|EDT|MDT|PDT)$/i.test(
            candidate,
          )
        ) {
          tripId = candidate;
    ...
    // 3. UI Error State Reset
    const errorEl = document.getElementById("trip-id-error");
    if (tripId && tripId.length >= 3) {
      if (errorEl) errorEl.style.display = "none";
    } else {
      if (errorEl) errorEl.style.display = "block";
    }

    // 4. Extract Origin & Destination (City, State)
    // Matching format: "Livonia, MI" or "Cicero, IL"
    const locations = [];
    const locationRegex =
      /([A-Za-z\s.]+),\s*([A-Z]{2})\b(?:\s*\d{5}(?:-\d{4})?)?/g;
    let locMatch;

    while ((locMatch = locationRegex.exec(cleanText)) !== null) {
      const cityState = `${locMatch[1].trim()}, ${locMatch[2]}`;
      // Exclude repetitive matches
      if (!locations.includes(cityState)) {
        locations.push(cityState);
      }
    }

    const origin = locations[0] || "Origin not set";
    const destination =
      locations[locations.length - 1] || "Destination not set";

    // 5. Extract Rate, Miles & Duration
    const rateMatch = cleanText.match(
      /(?:Rate|Pay|Gross|Tonu\s*(?:provided|approved)?)\s*[:=-]?\s*\$?(\d+(?:\.\d{1,2})?)/i,
    );
    const milesMatch = cleanText.match(/(\d+(?:\.\d+)?)\s*(?:mi|miles)/i);
    const durationMatch = cleanText.match(
      /(?:Duration|Time)\s*[:=-]?\s*(\d+d\s*\d+h|\d+h\s*\d+m|\d+h)/i,
    );

    return {
      tripId: tripId,
      gross: rateMatch ? parseFloat(rateMatch[1]) : 0,
      miles: milesMatch ? parseFloat(milesMatch[1]) : 0,
      duration: durationMatch ? durationMatch[1] : "--",
      origin: origin,
      destination: destination,
      isCancelled: /cancelled|tonu/i.test(cleanText),
    };
  }

  function parseTelegramText(text) {
    return parseUniversalTelegram(text);
  }
  ...
  async function saveParsedLoad() {
    ...
    if (!income.tripId || income.tripId === "N/A" || income.tripId.length < 3) {
      if (status) {
        status.style.display = "block";
        status.textContent = "ID рейса должен содержать минимум 3 символа.";
        status.classList.add("is-error");
      }
      return;
    }
    if (
      incomes.some(
        (entry) =>
          String(entry.tripId || "").toLowerCase() ===
          income.tripId.toLowerCase(),
      )
    ) {
      if (status) {
        status.style.display = "block";
        status.textContent = "Рейс с таким ID уже активен.";
        status.classList.add("is-error");
      }
      return;
    }
    ...
    if (status) {
      status.style.display = "block";
      status.textContent = `Загружен ${income.tripId || "груз"} на ${formatCurrency(income.gross)}.`;
      status.classList.remove("is-error");
    }
    switchTab("finance-section");
    showToast("Груз успешно загружен!");
  }
  ...
  async function runLoadOcr(file) {
    ...
    try {
      ...
      $("#paste-load-text")?.dispatchEvent(
        new Event("input", { bubbles: true }),
      );
      if (status)
        status.textContent =
          "Текст скриншота извлечён. Проверьте и сохраните.";
    } catch (error) {
      console.warn("Load OCR failed", error);
      if (status) {
        status.style.display = "block";
        status.textContent = "Ошибка OCR: не удалось прочитать скриншот.";
        status.classList.add("is-error");
      }
    }
  }
  ...
  function attachLoadParserHandlers() {
    ...
    dropzone.addEventListener("drop", (event) =>
      runLoadOcr(event.dataTransfer.files?.[0]),
    );
  ...
  function loadFinanceFromStorage() {
    ...
    const fin = raw
      ? JSON.parse(raw)
      : { id: "weekly", gross: 0, expenses: 0, net: 0, updatedAt: Date.now() };
    if (!localStorage.getItem(incomesKey) && Number(fin.gross) > 0)
      saveEntries(incomesKey, [makeEntry(Number(fin.gross), "Weekly gross")]);
    if (!localStorage.getItem(expensesKey) && Number(fin.expenses) > 0)
      saveEntries(expensesKey, [
        makeEntry(Number(fin.expenses), "Existing expenses"),
      ]);
    const incomes = loadEntries(incomesKey);
    const expenses = loadEntries(expensesKey);
    fin.gross = incomes.reduce(
      (sum, entry) => sum + parseAmount(entry.gross ?? entry.amount),
      0,
    );
    fin.expenses = expenses.reduce(
  ...
  function renderFinance(fin) {
    ...
    if (grossInput)
      grossInput.textContent = formatCurrency(Number(fin.gross || 0));
    if (expInput)
      expInput.textContent = formatCurrency(Number(fin.expenses || 0));
  ...
  function renderFinanceBreakdown() {
    ...
    const expenses = loadEntries(expensesKey).map((entry) => ({
      ...
      title: entry.category
        ? `${categoryIcons[entry.category] || ""} ${categoryLabels[entry.category] || entry.category}${entry.note ? ` - ${entry.note}` : ""}`.trim()
        : entry.label || (lang === "ru" ? "Расход" : "Expense"),
    }));
    ...
    const renderEntry = (entry) => {
      ...
      const entryRpm =
        entry.type === "income" && entryMiles > 0
          ? (parseAmount(entry.rpm) || entryGross / entryMiles).toFixed(2)
          : "";
      ...
      const formatTripDate = (value) => {
        ...
        const parsed = new Date(
          String(value).length === 10 && String(value).includes("-")
            ? `${value}T00:00:00`
            : value,
        );
        return Number.isNaN(parsed.getTime())
      ...
      if (entry.type === "income") {
        const fine = parseAmount(
          entry.fines?.find((item) => item.type === "Late PU")?.amount ??
            entry.driverMore?.latePuFine,
        );
        item.innerHTML = `
      ...
      deleteButton.setAttribute(
        "aria-label",
        entry.type === "income" ? labels.deleteIncome : labels.deleteExpense,
      );
      if (entry.type === "income") {
        deleteButton.classList.add("income-delete");
      } else {
  ...
  function editFinanceEntry(type, id) {
    ...
    if (type === "income") {
      ...
      $("#income-pickup-date").value = formatToInputDate(
        entry.pickupDate || entry.date,
      );
      $("#income-delivery-date").value = formatToInputDate(
        entry.deliveryDate || entry.date,
      );
  ...
  function updateIncomeRpm() {
    ...
    if (gross > 0 && miles > 0) {
      ...
      rpmLabel.textContent =
        getTranslation("income.rpm_calc") || "RPM: Add miles to calculate";
    }
  }
  ...
  function addIncomeStop(value = "") {
    ...
    input.placeholder =
      getTranslation("income.intermediate_placeholder") ||
      "Intermediate stop, e.g. Marinette, WI";
    input.value = value;
  ...
  async function handleAddIncome(event) {
    ...
    const tripId =
      $("#income-trip-id")?.value.trim() ||
      $("#income-load-number")?.value.trim() ||
      "N/A";
    if (tripId.length < 3) {
      alert(lang === "ru" ? "ID рейса должен содержать минимум 3 символа." : "Trip ID must contain at least 3 characters.");
      return;
    }
    const miles = parseAmount($("#income-miles")?.value);
    ...
    const incomes = loadEntries(incomesKey);
    const income = {
      id: editingFinanceEntry?.type === "income" ? editingFinanceEntry.id : uuidv4(),
      ...createLoadObject({
        tripId,
        ...
        extractedFinesArray:
          driverMore && parseAmount($("#fine-late-pu")?.value) > 0
            ? [
                {
                  type: "Late PU",
                  amount: parseAmount($("#fine-late-pu")?.value),
                },
              ]
            : [],
      }),
    ...
    if (
      incomes.some(
        (entry) =>
          entry.id !== income.id &&
          String(entry.tripId || "").toLowerCase() === tripId.toLowerCase(),
      )
    ) {
      alert("Рейс с таким ID рейса уже активен.");
      return;
    }
    if (editingFinanceEntry?.type === "income") {
      const index = incomes.findIndex(
        (entry) => entry.id === editingFinanceEntry.id,
      );
  ...
  async function deleteIncome(id) {
    const incomes = loadEntries(incomesKey);
    const target = incomes.find((entry) => entry.id === id);
    if (!target) return;
    const confirmed = confirm(`Удалить рейс ${target.tripId || target.id || "?"}?`);
    if (!confirmed) return;
    const filtered = incomes.filter((entry) => entry.id !== id);
    saveEntries(incomesKey, filtered);
    await persistFinance(loadFinanceFromStorage());
  }

  async function deleteExpense(id) {
    const confirmed = confirm("Удалить этот расход?");
    if (!confirmed) return;
    const expenses = loadEntries(expensesKey).filter((entry) => entry.id !== id);
    saveEntries(expensesKey, expenses);
    await persistFinance(loadFinanceFromStorage());
  }
  ...
  function verifyTabIsolation() {
        ...
        SECTIONS.forEach((childId) => {
          ...
          childSelectors.forEach((childSel) => {
            ...
            if (parentEl.contains(childEl)) {
              console.error(
                `Tab isolation violation: element ${childSel} is nested inside ${parentSel}`,
              );
    ...
    if (!ok) {
      console.warn(
        "verifyTabIsolation detected layout issues. Inspect elements with .tab-bleed-warning",
      );
    }
    return ok;
  }
    ...
    const check = () => {
      ...
      if (!storedLastRun) {
        ...
        localStorage.setItem(
          "weekly_archive_last_run",
          String(lastReset.getTime()),
        );
      ...
      document.documentElement.dataset.weeklyResetIn = String(
        Math.max(0, nextReset.getTime() - now.getTime()),
      );
    };
    check();
  ...
  function initReleaseNotification() {
    ...
    $("#whats-new-list").innerHTML = copy.updates
  ...
  async function openCameraScanner() {
    ...
    try {
      ...
      console.warn(
        "WebRTC camera unavailable, falling back to file input",
        err,
      );
  ...
  function attachScannerHandlers() {
    ...
    if (scanBolBtn)
      scanBolBtn.addEventListener("click", (e) => {
        e.preventDefault();
        openCameraScanner();
      });
    if (takePhotoBtn)
    ...
    if (quickDownloadBtn)
      quickDownloadBtn.addEventListener("click", downloadLatestPdf);
    if (quickShareBtn) quickShareBtn.addEventListener("click", shareLatestPdf);
    if (quickCloseBtn)
      quickCloseBtn.addEventListener("click", closeQuickShareModal);
    ...
    $("#driver-more-checkbox")?.addEventListener("change", (event) =>
      setDriverMoreState(event.target.checked),
    );
    ...
    $("#cancel-reset-finances-btn")?.addEventListener(
      "click",
      closeResetFinanceModal,
    );
    $("#confirm-reset-finances-btn")?.addEventListener(
      "click",
      confirmResetFinances,
    );
    ...
    $("#finance-history-list")?.addEventListener("click", async (event) => {
      ...
      if (button) {
        if (button.dataset.entryType === "income") {
          await deleteIncome(button.dataset.entryId);
        } else {
          await deleteExpense(button.dataset.entryId);
        }
        return;
      }
    ...
    if (resetFinancesBtn)
      resetFinancesBtn.addEventListener("click", resetFinances);
    if (dotTimerBtn)
      dotTimerBtn.addEventListener("click", () => {
        switchTab("finance-section");
        showRestTimerPanel();
  ...
  async function capturePhotoToPdf(opts = { save: false }) {
    if (opts.save && isProcessingUpload)
      return alert("A document is still being processed. Please wait.");
    ...
    try {
      ...
      if (opts.save) {
        try {
          ...
          console.warn("Saving PDF failed", e);
        }
      }
  ...
  function openDocSavedModal(doc) {
    ...
    if (subEl)
      subEl.textContent = `Created: ${formatDateTime(doc.ts || doc.date)}`;
    modal.setAttribute("aria-hidden", "false");
    // wire actions
  ...
  async function savePdfToStorage(blobOrData) {
    try {
      ...
      try {
        ...
        idbDocumentFallback = [
          newDoc,
          ...idbDocumentFallback.filter((doc) => doc.id !== newDoc.id),
        ];
        console.warn(
          "Document saved to IndexedDB because localStorage is full",
        );
      }
      // ensure UI updated
      renderArchive();
  ...
  function renderArchive() {
    ...
    if (!container) return;
    const docs = [...loadSavedDocs(), ...idbDocumentFallback].filter(
      (doc, index, all) =>
        all.findIndex((item) => item.id === doc.id) === index,
    );
    container.innerHTML = "";
    if (!docs.length) {
    ...
    docs.forEach((doc) => {
      ...
      card
        ...
        ?.addEventListener("click", () => openBase64Pdf(doc));
      card
        .querySelector(".btn-download")
        ?.addEventListener("click", () => downloadBase64Pdf(doc));
  ...
  async function shareLatestPdf() {
    if (!latestPdfBlob) return alert("No PDF available");
    try {
      const file = new File([latestPdfBlob], "BOL_Document.pdf", {
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          ...
          text: "Scanned BOL/POD",
        });
      } else if (navigator.share) {
        // some platforms accept URLs or text fallback
        await navigator.share({
          title: "Scanned Document",
          text: "Document saved. Please download to share.",
        });
  ...
  class RestTimer {
    ...
    saveState() {
      try {
        localStorage.setItem(
          this.storageKey,
          JSON.stringify({ remaining: this.remaining, running: this.running }),
        );
      } catch (e) {
        /* ignore */
      }
  ...
  function attachBuyFlow() {
    ...
    if (profileButton)
      profileButton.addEventListener("click", () =>
        showToast("Профиль водителя-гостя"),
      );
    ...
    const estimatorInputs = ["#fuel-price", "#route-miles", "#truck-mpg"]
      .map((sel) => $(sel))
      .filter(Boolean);
    const updateEstimate = () => {
      ...
      if (estimate)
        estimate.textContent = `Estimated fuel cost: ${formatCurrency((miles / mpg) * price)}`;
      if (savings)
        savings.textContent = `Estimated savings vs. 6 MPG: ${formatCurrency(Math.max(0, (miles / 6 - miles / mpg) * price))}`;
    };
    ...
    if (copyBtn) {
      copyBtn.addEventListener("click", async () => {
        try {
          ...
          alert(lang === "ru" ? "Не удалось скопировать — скопируйте вручную: 4023 0601 0098 7097" : "Unable to copy — please copy manually: 4023 0601 0098 7097");
        }
        ...
        const helpContents = {
          finance: {
            title: lang === "ru" ? "Финансовый отчет" : "Financial Summary",
            steps: lang === "ru" ? [
              "Введите Weekly Gross и расходы на топливо в поля.",
              "Приложение автоматически рассчитывает Чистую прибыль (Доход — Расходы).",
              "Все данные сохраняются локально на устройстве для офлайн-использования.",
            ] : [
          ...
          scan: {
            title: lang === "ru" ? "Сканирование BOL / POD" : "Scan BOL / POD",
            steps: lang === "ru" ? [
              "Нажмите Scan, чтобы открыть камеру и сделать фото накладной или акта приемки.",
              "Можно также загрузить фото с устройства, если поддерживается.",
              "Фотографии автоматически сжимаются для экономии места.",
            ] : [
          ...
          addExpense: {
            title: lang === "ru" ? "Добавить расходы" : "Add Fuel Expense",
            steps: lang === "ru" ? [
              "Нажмите Добавить расходы, чтобы быстро записать покупку топлива или дороги.",
              "Введите сумму — приложение автоматически вычтет её из Weekly Gross.",
              "Расходы и Чистый доход сохраняются локально, чтобы не потерять данные на дороге.",
            ] : [
          ...
          restTimer: {
            title: lang === "ru" ? "Таймер отдыха DOT" : "Rest Break Timer",
            steps: lang === "ru" ? [
              "Нажмите Таймер отдыха DOT, чтобы открыть 30-минутный обратный отсчёт.",
              "Используйте Start, Pause и Reset для управления перерывом.",
              "По истечении времени вы получите уведомление и вибрацию (на поддерживаемых устройствах).",
            ] : [
          ...
          diesel: {
            title: lang === "ru" ? "Пожертвование на дизель" : "Diesel Donation",
            steps: lang === "ru" ? [
              "Если хотите поддержать серверы, скопируйте номер Visa и отправьте небольшой перевод через банковское приложение.",
              "Нажмите Copy Card Number на экране благодарности, чтобы скопировать карту в буфер обмена.",
            ] : [
          ...
          pwaInstall: {
            title: lang === "ru" ? "Установить TruckerHub" : "Install TruckerHub",
            steps: lang === "ru" ? [
              "Нажмите Install Now, чтобы добавить TruckerHub на главный экран для быстрого доступа.",
              "Установленные приложения работают как нативные и могут работать офлайн.",
            ] : [
    ...
    if (declineClose)
      declineClose.addEventListener("click", () => close(decline));
  ...
  const translations = {
    en: {
      income: {
        title: "Add Income (Load)",
        trip_id: "Trip ID",
        load_num: "Load / Trip Number",
        gross_pay: "Gross Pay ($)",
        origin_loc: "Origin Location",
        add_stop: "+ Add intermediate stop",
        dest_loc: "Destination Location",
        pickup_date: "Pickup Date",
        delivery_date: "Delivery Date",
        trip_duration: "Trip Duration",
        total_miles: "Total Loaded Miles (optional)",
        rpm_calc: "RPM: Add miles to calculate",
        driver_more: "⚙️ Driver More / Fine Conditions",
        late_pu: "Late Pickup Penalty ($)",
        no_photos: "No PU/DEL Trailer Photos Penalty ($)",
        custom_notes: "Operational Warnings / Rules",
        cancel: "Cancel",
        save: "Save income",
        trip_id_placeholder: "T-112TK8HW1",
        load_num_placeholder: "#102",
        gross_placeholder: "0.00",
        origin_placeholder: "Chicago, IL",
        destination_placeholder: "Dallas, TX",
        duration_placeholder: "0d 12h",
        miles_placeholder: "270.76",
        late_pu_placeholder: "1000",
        no_photos_placeholder: "200",
        custom_notes_placeholder: "Enter custom notes or rules",
        intermediate_placeholder: "Intermediate stop, e.g. Marinette, WI",
      },
      financial: {
      ...
      install: {
        ...
        text: "Install this app for quick access and offline features.",
      },
      help: {
        financial: {
          title: "Financial Summary",
          text: "Tracks your weekly earnings. Enter Weekly Gross and Fuel Expenses; app calculates Net Pay.",
        },
        scan: {
          title: "Scan BOL / POD",
          text: "Capture photos of your Bill of Lading or Proof of Delivery and save as PDFs.",
        },
        expense: {
          title: "Add Fuel Expense",
          text: "Log fuel, toll, or repair costs to adjust weekly financial stats.",
        },
        timer: {
          title: "DOT Rest Break",
          text: "30-minute DOT rest break timer with alerts.",
        },
        diesel: {
          title: "Diesel Donation",
          text: "Copy Visa card number to support the app.",
        },
        pwa: {
          title: "Install App",
          text: "Add the app to your home screen for offline access.",
        },
      },
    },
    ru: {
      income: {
        title: "Добавить доход (груз)",
        trip_id: "ID рейса",
        load_num: "Номер груза / рейса",
        gross_pay: "Валовый доход ($)",
        origin_loc: "Пункт отправления",
        add_stop: "+ Добавить остановку",
        dest_loc: "Пункт назначения",
        pickup_date: "Дата загрузки",
        delivery_date: "Дата разгрузки",
        trip_duration: "Длительность рейса",
        total_miles: "Всего миль (необязательно)",
        rpm_calc: "RPM: Введите мили для расчета",
        driver_more: "⚙️ Штрафы и условия перевозки",
        late_pu: "Штраф за позднюю загрузку ($)",
        no_photos: "Штраф за фото при загрузке/разгрузке ($)",
        custom_notes: "Операционные предупреждения / правила",
        cancel: "Отмена",
        save: "Сохранить доход",
        trip_id_placeholder: "T-112TK8HW1",
        load_num_placeholder: "#102",
        gross_placeholder: "0,00",
        origin_placeholder: "Чикаго, IL",
        destination_placeholder: "Даллас, TX",
        duration_placeholder: "0д 12ч",
        miles_placeholder: "270,76",
        late_pu_placeholder: "1000",
        no_photos_placeholder: "200",
        custom_notes_placeholder: "Введите заметки или правила",
        intermediate_placeholder:
          "Промежуточная остановка, например Маринетт, WI",
      },
      financial: {
        title: "Финансовый отчет",
        weekly_gross: "Валовый доход",
        expenses: "Расходы",
        net_pay: "Чистая прибыль",
      },
      ...
      install: {
        ...
        text: "Установите приложение для быстрого доступа и офлайн-функций.",
      },
      help: {
        financial: {
          title: "Финансовый отчет",
          text: "Отслеживает еженедельный доход. Введите Weekly Gross и расходы на топливо; приложение рассчитывает чистый доход.",
        },
        scan: {
          title: "Сканирование BOL / POD",
          text: "Сделайте фото накладной или акта приемки и сохраните в PDF.",
        },
        expense: {
          title: "Добавить расходы",
          text: "Записывайте расходы на дизель, дороги и ремонт, чтобы учесть их в расчетах.",
        },
        timer: {
          title: "Таймер отдыха DOT",
          text: "30-минутный таймер перерыва DOT с оповещениями.",
        },
        diesel: {
          title: "Пожертвование на дизель",
          text: "Скопируйте номер карты Visa, чтобы поддержать приложение.",
        },
        pwa: {
          title: "Установить приложение",
          text: "Добавьте приложение на главный экран для работы офлайн.",
        },
      },
    },
  };
  ...
  function updateLanguage(lang) {
    ...
    $$("[data-i18n]").forEach((el) => {
      ...
      if (helpBtn) {
        ...
        if (textNode) textNode.nodeValue = String(text) + " ";
          ...
          el.insertBefore(document.createTextNode(String(text) + " "), helpBtn);
      } else {
        el.innerText = text;
      }
    });
    ...
    $$("[data-i18n-placeholder]").forEach((el) => {
      ...
      else if (node && node.title && typeof node.title === "string")
        el.placeholder = node.title;
    });
  }
  ...
  function renderTrips(filterText = "") {
    ...
    const results = trips.filter((t) => {
      ...
      return (
        ...
        pretty.includes(filterText)
      );
    });
    container.innerHTML = "";
    const lang = getSavedLang();
    if (!results.length) {
      container.innerHTML = lang === "ru" ? '<div class="empty-trips">Рейсы не найдены.</div>' : '<div class="empty-trips">No trips found.</div>';
      return;
    }
  ...
  function attachTripsHandlers() {
    ...
    if (input)
      input.addEventListener("input", (e) => renderTrips(e.target.value));
    if (dateIn)
      dateIn.addEventListener("change", (e) => renderTrips(e.target.value));
    // initial render
    renderTrips();
  }
